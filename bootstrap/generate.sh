...
export KUBERNETES_VERSION="${KUBERNETES_VERSION:-v1.15.7}"
...
# Outputs.
COMPONENTS_CLUSTER_API_GENERATED_FILE=${SOURCE_DIR}/cluster/provider-components/provider-components-cluster-api.yaml
...
PROVIDER_COMPONENTS_GENERATED_FILE=${OUTPUT_DIR}/../providers/provider-components.yaml
...
kustomize build "${SOURCE_DIR}/cluster/cluster" | envsubst > "${CLUSTER_GENERATED_FILE}"
...
curl -sSL https://github.com/kubernetes-sigs/cluster-api/releases/download/v0.2.9/cluster-api-components.yaml > "${COMPONENTS_CLUSTER_API_GENERATED_FILE}"
echo "Downloaded ${COMPONENTS_CLUSTER_API_GENERATED_FILE}"

# Generate Kubeadm Bootstrap Provider components file.
curl -sSL https://github.com/kubernetes-sigs/cluster-api-bootstrap-provider-kubeadm/releases/download/v0.1.5/bootstrap-components.yaml > "${COMPONENTS_KUBEADM_GENERATED_FILE}"
